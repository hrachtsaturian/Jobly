...
/** Middleware to use when they must be admin or that user.
 *
 * If not, raises Forbidden.
 */
...
function ensureHasAccessToTheUser(req, res, next) {
  try {
    ...
    const isTheSameUser = res.locals?.user?.username === req.params?.username;
    if (!isAdmin && !isTheSameUser) throw new ForbiddenError();
    return next();
  } catch (err) {
    return next(err);
  }
